...
import com.ibm.rational.clearcase.remote_core.cmds.*;
...
public class ClearcaseRCImpl extends BaseClearcase implements Clearcase {
	private static Logger log = Logger.getLogger(ClearcaseRCImpl.class);
	...
	private DirDiffUtil diffUtil = new DirDiffUtil(true);

	public ClearcaseRCImpl(Config config) {
		this.config = config;
		try {
	...
	public List<CCFile> diffPred(CCFile file) {
		...
		return diffUtil.diff(file.getFile(), files[0], files[1]);
	}
	...
	public Clearcase cloneForUser(User user) throws Exception {
		...
		ClearcaseRCImpl cc = getClass().newInstance();
		cc.config = config;
